...
setupInputToggle(change_amount_to_upload, word_input, choose, add);
...
add_whitelist_btn.addEventListener("click", () => {
  const term = new_whitelist_input.value.trim();
...
async function clear_blocked_words() {
...
add_whitelist_btn_tb.addEventListener("click", () => {
  const term = whitelist_input_tb.value.trim();
  if (!term) return;
...
// ===== Functions: Pagination / Filtering =====
