...
// AutoSaveBlockList broke the id adder for background.js

// ===== Event Listeners =====

choose.addEventListener("click", () => file_input.click());
setupInputToggle(change_amount_to_upload, site_input, choose, add);
...
site_input.addEventListener("input", () => {
  const query = site_input.value.toLowerCase();
  dropdown.innerHTML = "";
...
async function block_input() {
  ...
  if (
    ...
    normalizeRedirect(redirect_input.value, defaultRedirect) == defaultRedirect
  ) {
  ...
  for(const [block,config] of Object.entries(sites)){
    ...
    blocked_sites_temp[block] = config;
  }
...
function add_redirect() {
  const term = new_redirect_input.value.trim();
  if (!term || !current_site) return;
